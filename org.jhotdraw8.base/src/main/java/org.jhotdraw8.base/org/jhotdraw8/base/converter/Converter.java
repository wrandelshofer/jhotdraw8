...
public interface Converter<T> {
    ...
    ///     `Format of ⟨x⟩:`. Where `x` is the name of the production.
    ///   - A terminal symbol should be given with its literal characters.
    ...
    ///     `⟨x⟩`.
    ///   - Alternatives should be separated by full-width vertical bar:
    ///     `x｜y`.
    ///   - Groups of symbols should be surrounded by full width parantheses:
    ///     `（x y）`.
    ///   - Zero or one occurrences of a symbol should be surrounded by full
    ///     width square brackets: `［x］`.
    ///   - Zero or many occurrences of a symbol should be surrounded by full
