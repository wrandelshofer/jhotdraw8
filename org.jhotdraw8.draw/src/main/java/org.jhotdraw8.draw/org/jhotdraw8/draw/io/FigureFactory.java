...
public interface FigureFactory {
    ...
    /// @param f the figure
    /// @return the name
    /// @throws IOException if the factory does not support this figure
    ...
    /// @param key the key
    /// @return The name.
    /// @throws IOException if the factory does not support the key for
    ...
    /// may be different from the name defined in the key.
    ///
    /// The name can be an empty String `""` for the key returned by
    /// [#figureNodeListKeys] if the figure has exactly one node list key.
    ...
    /// the names returned by [#getElementNameByFigure].
    ///
    /// @param key   the key
    /// @param value the value
