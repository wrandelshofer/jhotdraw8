...
/// `DrawingModel` provides `DrawingModelEvent`s about a
/// `Drawing`.
///
...
///
/// The `DrawingModelEvent`s that a `DrawingModel` fires are based on
...
///
///
/// A `DrawingView` will only be updated properly, if all `Tool`s,
/// `Handle`s and inspectors update the drawing using the
/// `DrawingModel`.
///
public interface DrawingModel extends Observable, TreeModel<Figure> {
    ...
    /// Adds a listener for `DrawingModelEvent`s.
    ///
    ...
    /// Removes a listener for `DrawingModelEvent`s.
    ///
    /// @param l the listener
    ...
    /// Validates the model. This method is invoked by `DrawingView` each
