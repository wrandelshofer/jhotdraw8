...
public class BezierNodeEditHandle extends AbstractHandle {
    ...
    public BezierNodeEditHandle(Figure figure, MapAccessor<PersistentList<BezierNode>> pointKey, int pointIndex) {
        super(figure);
        ...
        if (pointKey != null) {
            if (event.getClickCount() == 1) {
                if (event.isControlDown() || event.isAltDown()) {
                    ...
                    dv.getModel().set(owner, pointKey, WrappedPersistentList.copyOf(path.getNodes()));
                    dv.recreateHandles();
                }
