...
public interface DrawingView extends WritableRenderContext {
    ...
    /// The drawing model.
    ///
    /// @return the drawing model property, with `getBean()` returning this
    /// drawing view, and `getName()` returning `DRAWING_PROPERTY`.
    ...
    /// `Node` of the tool to its tool panel which is stacked on top of the
    /// drawing panel. It then invokes `toolsetDrawingView(this)`.
    ///
    /// Setting a tool will removeChild the previous tool. The drawing view
    /// invokes `tool.setDrawingView(null)` and then removes its
    /// `Node` from its tool panel.
    ///
    /// @return the tool property, with `getBean()` returning this drawing
    /// view, and `getName()` returning `TOOL_PROPERTY`.
    ...
    /// The constrainer.
    ///
    /// @return the constrainer property, with `getBean()` returning this
    /// drawing view, and `getName()` returning
    ...
    /// focus.
    ///
    /// @return the focused property, with `getBean()` returning this
    /// drawing view, and `getName()` returning `FOCUSED_PROPERTY`.
    ...
    /// The handles.
    ///
    /// Note: The handles are represented by a `SequencedSet` because the
