...
/**
 * Base class for a {@link VertexSequenceBuilder} that uses a 'cost function'
 * and a 'back link' data structure.
 */
public abstract class AbstractCostAndBackLinksVertexSequenceBuilder<V, C extends Number & Comparable<C>> implements VertexSequenceBuilder<V, C> {
     ...
     */
    public AbstractCostAndBackLinksVertexSequenceBuilder(@NonNull C zero,
    ...
    public @Nullable OrderedPair<ImmutableList<V>, C> findVertexSequence(@NonNull Iterable<V> startVertices, @NonNull Predicate<V> goalPredicate, @NonNull C maxCost) {
        return toVertexSequence(search(startVertices, goalPredicate, zero, positiveInfinity, this.maxCost, nextVerticesFunction, costFunction, sumFunction), BackLink::getVertex);
    }
    ...
    protected static class BackLink<VV, CC extends Number & Comparable<CC>> {
