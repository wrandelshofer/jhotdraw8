/*
 * @(#)MultiArrayCsrGraphChunk16Bit.java
 ...
 * separate arrays.
 */
public class MultiArrayCsrGraphChunk16Bit implements GraphChunk16Bit {
    private static final boolean CLEAR_UNUSED_ELEMENTS = false;
    private static final int CLEAR_VALUE = 99;
    private static final char CLEAR_VALUE_16BIT = 99;
    ...
    public MultiArrayCsrGraphChunk16Bit(int vertexCount, final int initialArrowCapacity) {
        this.vertexCount = vertexCount;
        this.capacity = this.free = this.gapSize = initialArrowCapacity;
        this.gapIndex = 0;
        vertices = new int[vertexCount];
        sizes = new char[vertexCount];
        siblings = new char[initialArrowCapacity];
        arrows = new int[initialArrowCapacity];

        if (CLEAR_UNUSED_ELEMENTS) {
            Arrays.fill(vertices, CLEAR_VALUE);
            Arrays.fill(siblings, CLEAR_VALUE_16BIT);
    ...
    public int indexOf(final int v, final int u) {
        ...
        final int result = Arrays.binarySearch(siblings, from, to, (char) u);
        return result < 0 ? result + from : result - from;
    }

    @Override
    public char[] getSiblingsArray() {
        return siblings;
    }
    ...
    public void removeAllArrows(final int v) {
        ...
        if (CLEAR_UNUSED_ELEMENTS) {
            Arrays.fill(siblings, from, from + size + gapSize, CLEAR_VALUE_16BIT);
    ...
    void grow() {
        ...
        final char[] newSiblings = new char[newCapacity];
        ...
        if (CLEAR_UNUSED_ELEMENTS) {
            Arrays.fill(newSiblings, gapFromOffset, gapToOffset + deltaCapacity, CLEAR_VALUE_16BIT);
    ...
    void insertAtGap(final int u, final int data,
        ...
        siblings[from + insertionIndex] = (char) u;
    ...
    void insertBeforeGap(final int u, final int data,
        ...
        if (CLEAR_UNUSED_ELEMENTS) {
            Arrays.fill(siblings, to + 1, to + free, CLEAR_VALUE_16BIT);
    ...
    void insertAfterGap(final int u, final int data,
        ...
        // insert the element at insertion index
        siblings[from + insertionIndex - free] = (char) u;
        ...
        if (CLEAR_UNUSED_ELEMENTS) {
            Arrays.fill(siblings, to - free + 1, to, CLEAR_VALUE_16BIT);
