 ...
 */
public class UniqueShortestSequenceBuilder<V, A, C extends Number & Comparable<C>> extends AbstractShortestSequenceBuilder<V, A, C> {

    public UniqueShortestSequenceBuilder(@NonNull C zero, @NonNull C positiveInfinity, @NonNull C maxCost, @NonNull Function<V, Iterable<Arc<V, A>>> nextArcsFunction, @NonNull TriFunction<V, V, A, C> costFunction, @NonNull BiFunction<C, C, C> sumFunction) {
    ...
    protected @Nullable BackLink<V, A, C> search(
        ...
        while (!queue.isEmpty()) {
            ...
            for (Arc<V, A> entry : nextf.apply(u)) {
                ...
                C costThroughU = sumf.apply(costToU, costf.apply(u, v, a));
