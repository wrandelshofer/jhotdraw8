 ...
 */
public class UniqueOrOneHopBreadthFirstVertexSequenceBuilder<V, A> extends AbstractVertexSequenceBuilder<V> {

    public UniqueOrOneHopBreadthFirstVertexSequenceBuilder(@NonNull Function<V, Iterable<V>> nextNodesFunction) {
        this(Integer.MAX_VALUE, nextNodesFunction);
    }

    public UniqueOrOneHopBreadthFirstVertexSequenceBuilder(int maxLength, @NonNull Function<V, Iterable<V>> nextNodesFunction) {
    ...
    protected @Nullable BackLink<V> search(@NonNull Iterable<V> starts,
        ...
        for (V start : starts) {
            BackLink<V> rootBackLink = new BackLink<>(start, maxLength, null);
        ...
        while (!queue.isEmpty()) {
            ...
            if (node.getRemainingLength() > 0) {
                for (V next : nextNodesFunction.apply(node.getVertex())) {
                    if (visited.add(next)) {
                        BackLink<V> backLink = new BackLink<V>(next, node.getRemainingLength() - 1, node);
                        queue.add(backLink);
                    } else {
                        nonUnique.add(next);
                    }
