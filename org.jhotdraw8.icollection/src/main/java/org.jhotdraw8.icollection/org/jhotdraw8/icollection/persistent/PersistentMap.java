...
/// This interface provides copy-returning operations for a map.
///
...
public interface PersistentMap<K, V> extends ReadableMap<K, V> {
    ...
    PersistentMap<K, V> putting(K key, @Nullable V value);
    ...
    default PersistentMap<K, V> puttingAll(Map<? extends K, ? extends V> m) {
    ...
    default PersistentMap<K, V> puttingAll(java.lang.Iterable<? extends Map.Entry<? extends K, ? extends V>> c) {
        ...
        var s = this;
        for (var e : c) {
            s = s.putting(e.getKey(), e.getValue());
        }
        return s;
    }
    ...
    /// a different map instance with the entries added or updated
    @SuppressWarnings("unchecked")
    default PersistentMap<K, V> puttingKeyValues(Object... kv) {
        PersistentMap<K, V> that = this;
        for (int i = 0; i < kv.length; i += 2) {
            that = that.putting((K) kv[i], (V) kv[i + 1]);
        }
        return that;
    }
    ...
    PersistentMap<K, V> removing(K key);
    ...
    /// @return this map instance if it has not changed, or
    /// a different map instance with entries removed
    default PersistentMap<K, V> retainingAll(ReadableCollection<? extends K> c) {
        return retainingAll(c.asCollection());
    }
    ...
    /// @return
    default PersistentMap<K, V> merging(K key, V value,
