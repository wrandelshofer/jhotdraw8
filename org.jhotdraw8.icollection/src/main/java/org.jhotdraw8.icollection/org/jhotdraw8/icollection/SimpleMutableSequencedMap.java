 ...
 * <p>
 * See description at {@link SimpleImmutableSequencedMap}.
 * <p>
 * References:
 * <dl>
...
public class SimpleMutableSequencedMap<K, V> extends AbstractMutableChampMap<K, V, SequencedEntry<K, V>>
     ...
     */
    private @NonNull SimpleImmutableList<Object> vector;


    /**
     * Constructs a new empty map.
     */
    public SimpleMutableSequencedMap() {
        root = BitmapIndexedNode.emptyNode();
        vector = SimpleImmutableList.of();
    }
    ...
    public SimpleMutableSequencedMap(@NonNull Map<? extends K, ? extends V> c) {
        this((c instanceof SimpleMutableSequencedMap<?, ?> mvm)
                ? ((SimpleMutableSequencedMap<K, V>) mvm).toImmutable()
                : c.entrySet());
    }
     ...
     */
    @SuppressWarnings("unchecked")
    public SimpleMutableSequencedMap(@NonNull Iterable<? extends Entry<? extends K, ? extends V>> c) {
        if (c instanceof SimpleImmutableSequencedMap<?, ?>) {
            SimpleImmutableSequencedMap<K, V> that = (SimpleImmutableSequencedMap<K, V>) c;
            this.root = that.root;
            this.size = that.size;
            ...
            this.vector = SimpleImmutableList.of();
            putAll(c);
        }
     ...
     */
    @Override
    public @NonNull SimpleMutableSequencedMap<K, V> clone() {
        return (SimpleMutableSequencedMap<K, V>) super.clone();
    }
    ...
    public boolean putAll(@NonNull Iterable<? extends Entry<? extends K, ? extends V>> c) {
        if (c instanceof SimpleMutableSequencedMap<?, ?> that) {
            c = (Iterable<? extends Entry<? extends K, ? extends V>>) that.toImmutable();
        }
        if (isEmpty() && c instanceof SimpleImmutableSequencedMap<?, ?> that) {
            if (that.isEmpty()) {
                return false;
            }
     ...
     */
    public @NonNull SimpleImmutableSequencedMap<K, V> toImmutable() {
        owner = null;
        return size == 0 ? SimpleImmutableSequencedMap.of()
                : new SimpleImmutableSequencedMap<>(root, vector, size, offset);
    }
    ...
    private static class SerializationProxy<K, V> extends MapSerializationProxy<K, V> {
        ...
        protected @NonNull Object readResolve() {
            return new SimpleMutableSequencedMap<>(deserializedEntries);
        }
    }
}