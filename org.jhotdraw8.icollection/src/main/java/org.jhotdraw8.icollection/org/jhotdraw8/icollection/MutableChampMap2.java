 ...
 * <p>
 * See description at {@link ChampMap2}.
 * <p>
 * References:
 * <p>
 ...
 */
public class MutableChampMap2<K, V> extends AbstractMutableChampMap<K, V> {
    @Serial
     ...
     * @param m a map
     */
    public MutableChampMap2(@NonNull Map<? extends K, ? extends V> m) {
        if (m instanceof MutableChampMap2) {
            @SuppressWarnings("unchecked")
            MutableChampMap2<K, V> that = (MutableChampMap2<K, V>) m;
            this.root = that.root;
            this.size = that.size;
            this.modCount = 0;
        } else {
     ...
     * @param m an iterable
     */
    public MutableChampMap2(@NonNull Iterable<? extends Entry<? extends K, ? extends V>> m) {
        if (m instanceof ChampMap2) {
            @SuppressWarnings("unchecked")
            ChampMap2<K, V> that = (ChampMap2<K, V>) m;
            this.root = that.root;
            this.size = that.size;
        } else {
            this.root = BitmapIndexedNode.emptyNode();
     ...
     */
    @Override
    public @NonNull MutableChampMap2<K, V> clone() {
        return (MutableChampMap2<K, V>) super.clone();
    }
    ...
    public boolean containsKey(@Nullable Object o) {
        return root.findByKey((K) o,
                ChampMap2.keyHash(o), 0) != Node.NO_DATA;
    }
    ...
    public boolean putAll(@NonNull Iterable<? extends Entry<? extends K, ? extends V>> c) {
        if (c instanceof MutableChampMap2<?, ?> m) {
            c = (Iterable<? extends Entry<? extends K, ? extends V>>) m.toImmutable();
        }
        if (isEmpty() && c instanceof ChampMap2<?, ?> that) {
            if (that.isEmpty()) {
                return false;
            }
        ...
        if (c instanceof ChampMap2<?, ?> that) {
            var bulkChange = new BulkChangeEvent();
            var newRootNode = root.putAll(getOrCreateOwner(), (Node<SimpleImmutableEntry<K, V>>) (Node<?>) that.root, 0, bulkChange, ChampMap2::updateEntry, ChampMap2::entryKeyEquals,
                    ChampMap2::entryKeyHash, new ChangeEvent<>());
     ...
     */
    public @NonNull ChampMap2<K, V> toImmutable() {
        owner = null;
        return isEmpty() ? ChampMap2.of()
                : new ChampMap2<>(root, size);
    }
    ...
    private static class SerializationProxy<K, V> extends MapSerializationProxy<K, V> {
        ...
        protected @NonNull Object readResolve() {
            return new MutableChampMap2<>(deserializedEntries);
        }
    }
}