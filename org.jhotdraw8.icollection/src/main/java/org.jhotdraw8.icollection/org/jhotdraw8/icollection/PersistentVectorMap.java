...
public class PersistentVectorMap<K, V> implements PersistentSequencedMap<K, V>, Serializable {
    private static final PersistentVectorMap<?, ?> EMPTY = new PersistentVectorMap<>(
    ...
    PersistentVectorMap(BitmapIndexedNode<SequencedEntry<K, V>> hashMap,
    ...
    public static <K, V> PersistentVectorMap<K, V> copyOf(Iterable<? extends Map.Entry<? extends K, ? extends V>> map) {
        return new PersistentVectorMapBuilder<K, V>().addEntries(map).build();
    }
    ...
    /// @param <V> the value type
    /// @return a persistent copy
    public static <K, V> PersistentVectorMap<K, V> copyOf(Map<? extends K, ? extends V> map) {
        return new PersistentVectorMapBuilder<K, V>().addMap(map).build();
    }
    ...
    public static <K, V> PersistentVectorMap<K, V> of() {
        return (PersistentVectorMap<K, V>) PersistentVectorMap.EMPTY;
    }


    /// {@inheritDoc}
    @Override
    public PersistentVectorMap<K, V> cleared() {
    ...
    public boolean equals(@Nullable Object other) {
        ...
        if (other instanceof PersistentVectorMap<?, ?> that) {
    ...
    @Override
    public PersistentVectorMap<K, V> putting(K key, @Nullable V value) {
        return putLast(key, value, false);
    }

    @Override
    public PersistentVectorMap<K, V> puttingAll(Map<? extends K, ? extends V> m) {
        return (PersistentVectorMap<K, V>) PersistentSequencedMap.super.puttingAll(m);
    }

    @SuppressWarnings("unchecked")
    @Override
    public PersistentVectorMap<K, V> puttingAll(Iterable<? extends Map.Entry<? extends K, ? extends V>> c) {
        var m = toMutable();
        return m.putAll(c) ? m.toPersistent() : this;
    }

    @Override
    public PersistentVectorMap<K, V> puttingFirst(K key, @Nullable V value) {
        return putFirst(key, value, true);
    }

    private PersistentVectorMap<K, V> putFirst(K key, @Nullable V value, boolean moveToFirst) {
        ...
        if (details.isReplaced()
            ...
            return new PersistentVectorMap<>(newRoot, newVector.tree(), size, offset);
        }
    ...
    private PersistentVectorMap<K, V> putLast(K key, @Nullable V value, boolean moveToLast) {
        var details = new ChangeEvent<SequencedEntry<K, V>>();
    ...
    @Override
    public PersistentVectorMap<K, V> puttingLast(K key, @Nullable V value) {
    ...
    @Override
    public PersistentVectorMap<K, V> removing(K key) {
        ...
        if (details.isModified()) {
            ...
            return size == 1 ? PersistentVectorMap.of() : renumber(newRoot, result.tree(), size - 1, result.offset());
        }
        return this;
    }
    ...
    @Override
    public PersistentVectorMap<K, V> removingAll(Iterable<? extends K> c) {
        var t = toMutable();
        return t.removeAll(c) ? t.toPersistent() : this;
    }

    private PersistentVectorMap<K, V> renumber(
        ...
        if (SequencedData.vecMustRenumber(size, offset, this.vector.size())) {
            ...
            var b = new PersistentVectorMapBuilder<K, V>(new IdentityObject(), newOffset);
            ...
            return new PersistentVectorMap<>(tmp.hashMap, tmp.vector, size, newOffset);
        }
        return new PersistentVectorMap<>(root, vector, size, offset);
    }

    @Override
    public PersistentVectorMap<K, V> retainingAll(Iterable<? extends K> c) {
        var m = toMutable();
    ...
    private static class SerializationProxy<K, V> extends MapSerializationProxy<K, V> {
        ...
        protected Object readResolve() {
            return PersistentVectorMap.of().puttingAll(deserializedEntries);
        }
    }
