 ...
 */
public class SimpleMutableList<E> extends AbstractList<E> implements Serializable, ReadOnlyList<E>, List<E>, Cloneable {
    @Serial
    ...
    public boolean addAll(int index, Collection<? extends E> c) {
        ...
        SimpleImmutableList<E> immutable = toImmutable().addAll(index, c);
    ...
    public boolean removeAll(@NonNull Collection<?> c) {
        int oldSize = root.length;
        SimpleImmutableList<E> immutable = toImmutable().removeAll(c);
        if (oldSize != immutable.size()) {
            root = immutable.trie;
            modCount++;
            return true;
    ...
    public boolean retainAll(@NonNull Collection<?> c) {
        int oldSize = root.length;
        SimpleImmutableList<E> immutable = toImmutable().retainAll(c);
        if (oldSize != immutable.size()) {
            root = immutable.trie;
            modCount++;
            return true;
    ...
    public SimpleMutableList(@NonNull Iterable<? extends E> c) {
        if (c instanceof SimpleMutableList<?>) {
            c = ((SimpleMutableList<? extends E>) c).toImmutable();
        }
        if (c instanceof SimpleImmutableList<?>) {
            SimpleImmutableList<E> that = (SimpleImmutableList<E>) c;
            this.root = that.trie;
        } else {
    ...
    @NonNull
    public SimpleImmutableList<E> toImmutable() {
        return root.length == 0 ? SimpleImmutableList.of() : new SimpleImmutableList<>(root);
    }

    @Serial
    private @NonNull Object writeReplace() {
        return new SimpleMutableList.SerializationProxy<>(this);
    }
    ...
    @Override
    public SimpleMutableList<E> clone() {
        try {
            @SuppressWarnings("unchecked")
            SimpleMutableList<E> clone = (SimpleMutableList<E>) super.clone();
    ...
    private static class SerializationProxy<E> extends ListSerializationProxy<E> {
        ...
        protected @NonNull Object readResolve() {
            return new SimpleMutableList<>(deserializedElements);
        }
    }
}
