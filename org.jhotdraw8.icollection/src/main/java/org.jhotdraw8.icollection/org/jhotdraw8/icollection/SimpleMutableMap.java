 ...
 * <p>
 * See description at {@link SimpleImmutableMap}.
 * <p>
 * References:
 * <p>
 ...
 */
public class SimpleMutableMap<K, V> extends AbstractMutableChampMap<K, V> {
    @Serial
     ...
     * @param m a map
     */
    public SimpleMutableMap(@NonNull Map<? extends K, ? extends V> m) {
        if (m instanceof SimpleMutableMap) {
            @SuppressWarnings("unchecked")
            SimpleMutableMap<K, V> that = (SimpleMutableMap<K, V>) m;
            this.root = that.root;
            this.size = that.size;
            this.modCount = 0;
        } else {
     ...
     * @param m an iterable
     */
    public SimpleMutableMap(@NonNull Iterable<? extends Entry<? extends K, ? extends V>> m) {
        if (m instanceof SimpleImmutableMap) {
            @SuppressWarnings("unchecked")
            SimpleImmutableMap<K, V> that = (SimpleImmutableMap<K, V>) m;
            this.root = that.root;
            this.size = that.size;
        } else {
            this.root = BitmapIndexedNode.emptyNode();
     ...
     */
    @Override
    public @NonNull SimpleMutableMap<K, V> clone() {
        return (SimpleMutableMap<K, V>) super.clone();
    }
    ...
    public boolean containsKey(@Nullable Object o) {
        return root.findByKey((K) o,
                SimpleImmutableMap.keyHash(o), 0) != Node.NO_DATA;
    }
    ...
    public @Nullable V get(Object o) {
        Object result = root.findByKey((K) o,
                SimpleImmutableMap.keyHash(o), 0);
    ...
    public boolean putAll(@NonNull Iterable<? extends Entry<? extends K, ? extends V>> c) {
        if (c instanceof SimpleMutableMap<?, ?> m) {
    ...
    ChangeEvent<V> putEntry(@Nullable K key, @Nullable V val) {
        int keyHash = SimpleImmutableMap.keyHash(key);
        ChangeEvent<V> details = new ChangeEvent<>();
        root = root.put(getOrCreateOwner(), key, val, keyHash, 0, details, SimpleImmutableMap::keyHash);
     ...
     */
    public @NonNull SimpleImmutableMap<K, V> toImmutable() {
        owner = null;
        return isEmpty() ? SimpleImmutableMap.of()
                : new SimpleImmutableMap<>(root, size);
    }
    ...
    private static class SerializationProxy<K, V> extends MapSerializationProxy<K, V> {
        ...
        protected @NonNull Object readResolve() {
            return new SimpleMutableMap<>(deserializedEntries);
        }
    }
}