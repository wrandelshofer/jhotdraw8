...
public class EntryIterator<K, V, E> implements Iterator<E> {
    ...
    private static final int ENTRY_SIZE = TrieNode.ENTRY_SIZE;
    private final BiFunction<K, V, E> mapper;

    public EntryIterator(TrieNode<K, V> node, BiFunction<K, V, E> mapper) {
        this.mapper = mapper;
        push(node);
    }
    ...
    private void moveToNextNode() {
        ...
        if (nodeCount > 0) {
            ...
            TrieNode<K, V> node = (TrieNode<K, V>) nodeArray[nodeArray.length - nodeCount];
            if (nodeCount == 1) pop();
            push(node);
        } else {
            pop();
    ...
    private void push(TrieNode<K, V> node) {
        ...
        this.entryCount = node.collisionEntryCount();
    }
