...
public interface RedBlackTree<K, V> extends Iterable<Node<K, V>> {
    ...
    /// Adds all the elements of the given `tree` to this tree, if not already present.
    ///
    ...
    /// Returns the least `Node` in this tree greater than or equal to
    /// the given element, or `Empty` if there is no such element.
    RedBlackTree<K, V> ceiling(K e, Comparator<? super K> comparator);

    /// Returns the greatest `Node` in this tree less than or equal to
    /// the given element, or `Empty` if there is no such element.
    RedBlackTree<K, V> floor(K e, Comparator<? super K> comparator);

    /// Returns the least `Node` in this tree strictly greater than the
    /// given element, or `Empty` if there is no such element.
    RedBlackTree<K, V> higher(K e, Comparator<? super K> comparator);

    /// Returns the greatest `Node` in this tree strictly less than the
    /// given element, or `Empty` if there is no such element.
