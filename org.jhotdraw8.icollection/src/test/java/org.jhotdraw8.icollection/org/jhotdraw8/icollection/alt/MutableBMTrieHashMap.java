...
///
/// See description at [PersistentBMTrieHashMap].
///
/// References:
/// <dl>
...
public class MutableBMTrieHashMap<K, V> extends AbstractMutableChampMap<K, V, SequencedEntry<K, V>>
    ...
    public MutableBMTrieHashMap(Map<? extends K, ? extends V> c) {
        this((c instanceof MutableBMTrieHashMap<?, ?> mvm)
                ? ((MutableBMTrieHashMap<K, V>) mvm).toPersistent()
                : c.entrySet());
    }
    ...
    public MutableBMTrieHashMap(Iterable<? extends Entry<? extends K, ? extends V>> c) {
        if (c instanceof PersistentBMTrieHashMap<?, ?>) {
            PersistentBMTrieHashMap<K, V> that = (PersistentBMTrieHashMap<K, V>) c;
            this.root = that.root;
            this.size = that.size;
    ...
    @Override
    public MutableBMTrieHashMap<K, V> clone() {
        return (MutableBMTrieHashMap<K, V>) super.clone();
    }
    ...
    public boolean putAll(Iterable<? extends Entry<? extends K, ? extends V>> c) {
        if (c instanceof MutableBMTrieHashMap<?, ?> that) {
            c = (Iterable<? extends Entry<? extends K, ? extends V>>) that.toPersistent();
        }
        if (isEmpty() && c instanceof PersistentBMTrieHashMap<?, ?> that) {
            if (that.isEmpty()) {
                return false;
            }
    ...
    public PersistentBMTrieHashMap<K, V> toPersistent() {
        ...
        return size == 0 ? PersistentBMTrieHashMap.of()
                : new PersistentBMTrieHashMap<>(root, vector, size, offset);
    }
    ...
    private static class SerializationProxy<K, V> extends MapSerializationProxy<K, V> {
        ...
        protected Object readResolve() {
            return new MutableBMTrieHashMap<>(deserializedEntries);
        }
    }
}