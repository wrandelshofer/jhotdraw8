...
///
/// See description at [PersistentBMTrieSet].
///
/// References:
/// <dl>
...
public class MutableBMTrieSet<E> extends AbstractMutableChampSet<E, SequencedElement<E>> implements ReadableSequencedSet<E>,
        SequencedSet<E> {
    @Serial
    ...
    public MutableBMTrieSet(Iterable<? extends E> c) {
        if (c instanceof MutableBMTrieSet<?>) {
            c = ((MutableBMTrieSet<? extends E>) c).toPersistent();
        }
        if (c instanceof PersistentBMTrieSet<?>) {
            PersistentBMTrieSet<E> that = (PersistentBMTrieSet<E>) c;
            this.root = that.root;
            this.size = that.size;
    ...
    @Override
    public MutableBMTrieSet<E> clone() {
        return (MutableBMTrieSet<E>) super.clone();
    }
...
*/
    ...
    public PersistentBMTrieSet<E> toPersistent() {
        owner = null;
        return size == 0
                ? PersistentBMTrieSet.of()
                : new PersistentBMTrieSet<>(root, vector, size, offset);
    }
    ...
    private static class SerializationProxy<E> extends SetSerializationProxy<E> {
        ...
        protected Object readResolve() {
            return new MutableBMTrieSet<>(deserializedElements);
        }
    }
}