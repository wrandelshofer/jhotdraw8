...
///
/// See description at [PersistentBMTrieHashSet].
///
/// References:
/// <dl>
...
public class MutableBMTrieHashSet<E> extends AbstractMutableChampSet<E, SequencedElement<E>> implements ReadableSequencedSet<E>,
        SequencedSet<E> {
    @Serial
    ...
    public MutableBMTrieHashSet(Iterable<? extends E> c) {
        if (c instanceof MutableBMTrieHashSet<?>) {
            c = ((MutableBMTrieHashSet<? extends E>) c).toPersistent();
        }
        if (c instanceof PersistentBMTrieHashSet<?>) {
            PersistentBMTrieHashSet<E> that = (PersistentBMTrieHashSet<E>) c;
            this.root = that.root;
            this.size = that.size;
    ...
    @Override
    public MutableBMTrieHashSet<E> clone() {
        return (MutableBMTrieHashSet<E>) super.clone();
    }
...
*/
    ...
    public PersistentBMTrieHashSet<E> toPersistent() {
        owner = null;
        return size == 0
                ? PersistentBMTrieHashSet.of()
                : new PersistentBMTrieHashSet<>(root, vector, size, offset);
    }
    ...
    private static class SerializationProxy<E> extends SetSerializationProxy<E> {
        ...
        protected Object readResolve() {
            return new MutableBMTrieHashSet<>(deserializedElements);
        }
    }
}