...
public class PersistentBMTrieHashMap<K, V> implements PersistentSequencedMap<K, V>, Serializable {
    private static final PersistentBMTrieHashMap<?, ?> EMPTY = new PersistentBMTrieHashMap<>(
    ...
    protected PersistentBMTrieHashMap(PrivateData privateData) {
    ...
    protected PersistentBMTrieHashMap<K, V> newInstance(PrivateData privateData) {
        return new PersistentBMTrieHashMap<>(privateData);
    }

    private PersistentBMTrieHashMap<K, V> newInstance(BitmapIndexedNode<SequencedEntry<K, V>> root,
                                                      PersistentBMTrieList<Object> vector,
                                                      int size, int offset) {
        return new PersistentBMTrieHashMap<>(new PrivateData(new OpaqueRecord<>(root, vector, size, offset)));
    }

    PersistentBMTrieHashMap(BitmapIndexedNode<SequencedEntry<K, V>> root,
                            PersistentBMTrieList<Object> vector,
                            int size, int offset) {
        this.root = root;
        this.size = size;
    ...
    @SuppressWarnings("unchecked")
    public static <K, V> PersistentBMTrieHashMap<K, V> of() {
        return (PersistentBMTrieHashMap<K, V>) PersistentBMTrieHashMap.EMPTY;
    }


    /// {@inheritDoc}
    @Override
    public PersistentBMTrieHashMap<K, V> cleared() {
    ...
    public boolean equals(@Nullable Object other) {
        ...
        if (other instanceof PersistentBMTrieHashMap<?, ?> that) {
    ...
    @Override
    public PersistentBMTrieHashMap<K, V> putting(K key, @Nullable V value) {
        return putLast(key, value, false);
    }

    @Override
    public PersistentBMTrieHashMap<K, V> puttingAll(Map<? extends K, ? extends V> m) {
        return (PersistentBMTrieHashMap<K, V>) PersistentSequencedMap.super.puttingAll(m);
    }

    @SuppressWarnings("unchecked")
    @Override
    public PersistentBMTrieHashMap<K, V> puttingAll(Iterable<? extends Map.Entry<? extends K, ? extends V>> c) {
        var m = toMutable();
        return m.putAll(c) ? m.toPersistent() : this;
    }

    @Override
    public PersistentBMTrieHashMap<K, V> puttingFirst(K key, @Nullable V value) {
        return putFirst(key, value, true);
    }

    private PersistentBMTrieHashMap<K, V> putFirst(K key, @Nullable V value, boolean moveToFirst) {
        ...
        if (details.isReplaced()
            ...
            var newVector = vector.settingAt(details.getNewDataNonNull().getSequenceNumber() - offset, details.getNewDataNonNull());
    ...
    private PersistentBMTrieHashMap<K, V> putLast(K key, @Nullable V value, boolean moveToLast) {
        var details = new ChangeEvent<SequencedEntry<K, V>>();
    ...
    @Override
    public PersistentBMTrieHashMap<K, V> puttingLast(K key, @Nullable V value) {
    ...
    @Override
    public PersistentBMTrieHashMap<K, V> removing(K key) {
        ...
        if (details.isModified()) {
            ...
            return size == 1 ? PersistentBMTrieHashMap.of() : renumber(newRoot, result.first(), size - 1, result.second());
        }
        return this;
    }
    ...
    @Override
    public PersistentBMTrieHashMap<K, V> removingAll(Iterable<? extends K> c) {
        var t = toMutable();
        return t.removeAll(c) ? t.toPersistent() : this;
    }

    private PersistentBMTrieHashMap<K, V> renumber(
    ...
    @Override
    public PersistentBMTrieHashMap<K, V> retainingAll(Iterable<? extends K> c) {
        var m = toMutable();
    ...
    @Override
    public MutableBMTrieHashMap<K, V> toMutable() {
        return new MutableBMTrieHashMap<>(this);
    }

    @Override
    public MutableBMTrieHashMap<K, V> asMap() {
        return new MutableBMTrieHashMap<>(this);
    }
    ...
    private static class SerializationProxy<K, V> extends MapSerializationProxy<K, V> {
        ...
        protected Object readResolve() {
            return PersistentBMTrieHashMap.of().puttingAll(deserializedEntries);
        }
    }
