...
public class MutableBMTrieList<E> extends AbstractList<E> implements Serializable, ReadableList<E>, List<E>, Cloneable {
    @Serial
    ...
    public boolean addAll(int index, Collection<? extends E> c) {
        ...
        PersistentBMTrieList<E> persistent = toPersistent().addingAllAt(index, c);
    ...
    public boolean removeAll(Collection<?> c) {
        int oldSize = root.length;
        PersistentBMTrieList<E> persistent = toPersistent().removingAll(c);
        if (oldSize != persistent.size()) {
            root = persistent.root;
            modCount++;
            return true;
    ...
    public boolean retainAll(Collection<?> c) {
        int oldSize = root.length;
        PersistentBMTrieList<E> persistent = toPersistent().retainingAll(c);
        if (oldSize != persistent.size()) {
            root = persistent.root;
            modCount++;
            return true;
    ...
    public MutableBMTrieList(Iterable<? extends E> c) {
        if (c instanceof MutableBMTrieList<?>) {
            c = ((MutableBMTrieList<? extends E>) c).toPersistent();
        }
        if (c instanceof PersistentBMTrieList<?>) {
            PersistentBMTrieList<E> that = (PersistentBMTrieList<E>) c;
            this.root = that.root;
        } else {
    ...
    public PersistentBMTrieList<E> toPersistent() {
        return root.length == 0 ? PersistentBMTrieList.of() : new PersistentBMTrieList<>(root);
    }
    ...
    @Override
    public final MutableBMTrieList<E> clone() {
        try {
            @SuppressWarnings("unchecked")
            MutableBMTrieList<E> clone = (MutableBMTrieList<E>) super.clone();
            return clone;
    ...
    private static class SerializationProxy<E> extends ListSerializationProxy<E> {
        ...
        protected Object readResolve() {
            return new MutableBMTrieList<>(deserializedElements);
        }
    }
}
