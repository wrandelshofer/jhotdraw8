...
public class PersistentBMTrieMap<K, V> implements PersistentSequencedMap<K, V>, Serializable {
    private static final PersistentBMTrieMap<?, ?> EMPTY = new PersistentBMTrieMap<>(
    ...
    protected PersistentBMTrieMap(PrivateData privateData) {
    ...
    protected PersistentBMTrieMap<K, V> newInstance(PrivateData privateData) {
        return new PersistentBMTrieMap<>(privateData);
    }

    private PersistentBMTrieMap<K, V> newInstance(BitmapIndexedNode<SequencedEntry<K, V>> root,
                                                  PersistentBMTrieList<Object> vector,
                                                  int size, int offset) {
        return new PersistentBMTrieMap<>(new PrivateData(new OpaqueRecord<>(root, vector, size, offset)));
    }

    PersistentBMTrieMap(BitmapIndexedNode<SequencedEntry<K, V>> root,
                        PersistentBMTrieList<Object> vector,
                        int size, int offset) {
        this.root = root;
        this.size = size;
    ...
    /// @param <V> the value type
    /// @return a persistent copy
    public static <K, V> PersistentBMTrieMap<K, V> copyOf(Map<? extends K, ? extends V> map) {
        return PersistentBMTrieMap.<K, V>of().puttingAll(map);
    }
    ...
    @SuppressWarnings("unchecked")
    public static <K, V> PersistentBMTrieMap<K, V> of() {
        return (PersistentBMTrieMap<K, V>) PersistentBMTrieMap.EMPTY;
    }


    /// {@inheritDoc}
    @Override
    public PersistentBMTrieMap<K, V> cleared() {
    ...
    public boolean equals(@Nullable Object other) {
        ...
        if (other instanceof PersistentBMTrieMap<?, ?> that) {
    ...
    @Override
    public PersistentBMTrieMap<K, V> putting(K key, @Nullable V value) {
        return putLast(key, value, false);
    }

    @Override
    public PersistentBMTrieMap<K, V> puttingAll(Map<? extends K, ? extends V> m) {
        return (PersistentBMTrieMap<K, V>) PersistentSequencedMap.super.puttingAll(m);
    }
    ...
    @Override
    public PersistentBMTrieMap<K, V> puttingFirst(K key, @Nullable V value) {
        return putFirst(key, value, true);
    }

    private PersistentBMTrieMap<K, V> putFirst(K key, @Nullable V value, boolean moveToFirst) {
    ...
    private PersistentBMTrieMap<K, V> putLast(K key, @Nullable V value, boolean moveToLast) {
        var details = new ChangeEvent<SequencedEntry<K, V>>();
    ...
    @Override
    public PersistentBMTrieMap<K, V> puttingLast(K key, @Nullable V value) {
    ...
    @Override
    public PersistentBMTrieMap<K, V> removing(K key) {
        ...
        if (details.isModified()) {
            ...
            return size == 1 ? PersistentBMTrieMap.of() : renumber(newRoot, result.first(), size - 1, result.second());
        }
        return this;
    }
    ...
    private PersistentBMTrieMap<K, V> renumber(
    ...
    @Override
    public MutableBMTrieMap<K, V> toMutable() {
        return new MutableBMTrieMap<>(this);
    }

    @Override
    public MutableBMTrieMap<K, V> asMap() {
        return new MutableBMTrieMap<>(this);
    }
    ...
    private static class SerializationProxy<K, V> extends MapSerializationProxy<K, V> {
        ...
        protected Object readResolve() {
            return PersistentBMTrieMap.of().puttingAll(deserializedEntries);
        }
    }
