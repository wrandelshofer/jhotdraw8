 ...
 * <dl>
 *     <dt>BitMappedTrie.java. Copyright 2023 (c) vavr. Apache License Version 2.0.</dt>
 ...
 * </dl>
 */
class BitMappedTrie<T> implements Serializable {
    ...
    BitMappedTrie<T> prepend(java.util.Iterator<? extends T> iterator, int size) {
        BitMappedTrie<T> result = this;
        while (size > 0) {
    ...
    Iterator<T> iterator(int fromIndex) {
        return new Iterator<T>() {
            private final int globalLength = BitMappedTrie.this.length;
            private int globalIndex = fromIndex;

            private int index = lastDigit(offset + fromIndex);
