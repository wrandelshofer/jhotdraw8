 ...
 * Hash-Array Mapped Prefix-trie (CHAMP) with a bit-partition size of 6.
 * <p>
 ...
 */
public class PersistentTrie6Set<E> extends AbstractReadOnlySet<E> implements PersistentSet<E> {

    private static final Node<?> EMPTY_NODE = new BitmapIndexedNode<>(null, 0, 0, new Object[]{});

    private static final PersistentTrie6Set<?> EMPTY_SET = new PersistentTrie6Set<>(EMPTY_NODE, 0, 0);

    private final Node<E> root;
    private final int hashCode;
    private final int size;

    private PersistentTrie6Set(Node<E> root, int hashCode, int size) {
        this.root = root;
        this.hashCode = hashCode;
        this.size = size;
    }

    public static <K> PersistentTrie6Set<K> copyOf(@NonNull Iterable<? extends K> set) {
        if (set instanceof PersistentTrie6Set) {
            //noinspection unchecked
            return (PersistentTrie6Set<K>) set;
        }
    ...
    private static <K> Node<K> emptyNode() {
        return (Node<K>) PersistentTrie6Set.EMPTY_NODE;
    }

    @SafeVarargs
    public static <K> PersistentTrie6Set<K> of(@NonNull K... keys) {
        return PersistentTrie6Set.<K>of().copyAddAll(Arrays.asList(keys));
    }

    @SuppressWarnings("unchecked")
    public static <K> PersistentTrie6Set<K> of() {
        return (PersistentTrie6Set<K>) PersistentTrie6Set.EMPTY_SET;
    }
    ...
    public boolean equals(final Object other) {
        ...
        if (other instanceof PersistentTrie6Set) {
            PersistentTrie6Set<?> that = (PersistentTrie6Set<?>) other;
    ...
    public @NonNull PersistentTrie6Set<E> copyAdd(final @NonNull E key) {
        ...
        if (changeEvent.isModified) {
            return new PersistentTrie6Set<>(newRootNode, hashCode + keyHash, size + 1);
        }

        return this;
    }

    public @NonNull PersistentTrie6Set<E> copyAddAll(final @NonNull Iterable<? extends E> set) {
    ...
    public @NonNull PersistentTrie6Set<E> copyRemove(final @NonNull E key) {
        final int keyHash = Objects.hashCode(key);
        final ChangeEvent changeEvent = new ChangeEvent();
    ...
    public @NonNull PersistentTrie6Set<E> copyRemoveAll(final @NonNull Iterable<? extends E> set) {
        final TransientTrieSet<E> t = this.asTransient();
        boolean modified = false;
        for (final E key : set) {
    ...
    public @NonNull PersistentTrie6Set<E> copyRetainAll(final @NonNull Collection<? extends E> set) {
        final TransientTrieSet<E> t = this.asTransient();
        boolean modified = false;
        for (E key : this) {
            if (!set.contains(key)) {
    ...
    private static abstract class Node<K> {
        ...
        static final int BIT_PARTITION_SIZE = 6;
        static final int BIT_PARTITION_MASK = 0b11_1111;
        ...
        static long bitpos(final int mask) {
            return 1L << mask;
        }

        static int index(final long bitmap, final long bitpos) {
            return Long.bitCount(bitmap & (bitpos - 1));
        }

        static int index(final long bitmap, final int mask, final long bitpos) {
    ...
    private static final class BitmapIndexedNode<K> extends Node<K> {
        ...
        BitmapIndexedNode(final @Nullable PersistentTrieHelper.Nonce bulkEdit, final long nodeMap,
                          final long dataMap, final Object[] nodes) {
            super(bulkEdit);
        ...
        boolean contains(final K key, final int keyHash, final int shift) {
            ...
            final long bitpos = bitpos(mask);

            final long dataMap = dataMap();
            if ((dataMap & bitpos) != 0) {
                final int index = index(dataMap, mask, bitpos);
                return Objects.equals(getKey(index), key);
            }

            final long nodeMap = nodeMap();
            if ((nodeMap & bitpos) != 0) {
        ...
        Node<K> copyAndInsertValue(final PersistentTrieHelper.Nonce bulkEdit, final long bitpos,
                                   final K key) {
        ...
        private BitmapIndexedNode<K> copyOrUpdate(final @Nullable PersistentTrieHelper.Nonce bulkEdit, final long nodeMap,
                                                  final long dataMap, final Object[] nodes) {
        ...
        Node<K> copyAndMigrateFromInlineToNode(final PersistentTrieHelper.Nonce bulkEdit,
                                               final long bitpos, final Node<K> node) {
        ...
        Node<K> copyAndRemoveValue(final PersistentTrieHelper.Nonce bulkEdit, final long bitpos) {
            final int idx = TUPLE_LENGTH * dataIndex(bitpos);
        ...
        Node<K> copyAndSetNode(final PersistentTrieHelper.Nonce bulkEdit, final long bitpos,
        ...
        int dataIndex(final long bitpos) {
            return Long.bitCount(dataMap() & (bitpos - 1));
        }
        ...
        @Override
        int nodeArity() {
            return Long.bitCount(nodeMap());
        }

        Node<K> nodeAt(final long bitpos) {
            return getNode(nodeIndex(bitpos));
        }

        int nodeIndex(final long bitpos) {
            return Long.bitCount(nodeMap() & (bitpos - 1));
        }
        ...
        private Node<K> mergeTwoKeyValPairs(PersistentTrieHelper.Nonce bulkEdit,
            ...
            if (mask0 != mask1) {
                ...
                final long dataMap = bitpos(mask0) | bitpos(mask1);
                if (mask0 < mask1) {
                ...
                final long nodeMap = bitpos(mask0);
    ...
    private static class TransientTrieSet<K> {
        ...
        TransientTrieSet(PersistentTrie6Set<K> trieSet) {
        ...
        public PersistentTrie6Set<K> freeze() {
            bulkEdit = new PersistentTrieHelper.Nonce();
            return size == 0 ? PersistentTrie6Set.of() : new PersistentTrie6Set<>(root, hashCode, size);
        }
    }
