...
/**
 * Wraps {@code Map} functions into the {@link java.util.Map} API.
 *
...
public class MapWrapper<K, V> extends AbstractMap<K, V> {
    ...
    public MapWrapper(ReadOnlyMap<K, V> m) {
        this(m::entries, m::size, m::containsKey, m::get, null, null, null);
    }

    public MapWrapper(@NonNull Supplier<Iterator<Entry<K, V>>> iteratorFunction,
    ...
    public @NonNull Set<Entry<K, V>> entrySet() {
        return new SetWrapper<Entry<K, V>>(
                iteratorFunction,
                sizeFunction,
                this::containsEntry,
                clearFunction,
