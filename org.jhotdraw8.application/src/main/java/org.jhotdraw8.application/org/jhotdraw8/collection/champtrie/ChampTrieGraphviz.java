...
public class ChampTrieGraphviz<K, V> {
        ...
        for (int mask = 0; mask <= Node.BIT_PARTITION_MASK; mask++) {
            ...
            if (((nodeMap | dataMap) & bitpos) != 0) {
                ...
                if ((dataMap & bitpos) != 0) {
                    ...
                    if (printValue) {
                        a.append('=');
                        a.append(Objects.toString(node.getValue(Node.index(dataMap, bitpos), entryLength)));
                    }
                    if (printSequenceNumber) {
                        a.append(" #");
                        a.append(Long.toString(node.getSequenceNumber(Node.index(dataMap, bitpos), entryLength)));
                    }
                } else {
                    a.append("·");
                }
    ...
    private void dumpHashCollisionNodeSubTree(Appendable a, HashCollisionNode<K, V> node, int entryLength, boolean printValue, boolean printSequenceNumber, int shift, int keyHash) throws IOException {
        ...
        @NonNull Object[] nodes = node.nodes;
