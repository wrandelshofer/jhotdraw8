...
public class WrappedReadOnlyMap<K, V> implements ReadOnlyMap<K, V> {
    ...
    public WrappedReadOnlyMap(@NonNull ReadOnlyMap<K, V> m) {
        this(m::iterator, m::size, m::containsKey, m::get);
    }

    public WrappedReadOnlyMap(@NonNull Map<K, V> m) {
        this(() -> m.entrySet().iterator(), m::size, m::containsKey, m::get);
    }

    public WrappedReadOnlyMap(@NonNull Supplier<Iterator<Map.Entry<K, V>>> iteratorFunction,
