 ...
 */
public class PersistentTrie6Map<K, V> extends AbstractReadOnlyMap<K, V> implements PersistentMap<K, V> {

    private static final Node<?, ?> EMPTY_NODE = new BitmapIndexedNode<>(null, (0), (0), new Object[]{});

    private static final PersistentTrie6Map<?, ?> EMPTY_MAP = new PersistentTrie6Map<>(EMPTY_NODE, 0, 0);

    private final @NonNull Node<K, V> root;
    private final int hashCode;
    private final int size;

    private PersistentTrie6Map(@NonNull Node<K, V> root, int hashCode, int size) {
        this.root = root;
        this.hashCode = hashCode;
        this.size = size;
    }

    public static <K, V> PersistentTrie6Map<K, V> of(@NonNull ReadOnlyMap<? extends K, ? extends V> map) {
        if (map instanceof PersistentTrie6Map) {
            @SuppressWarnings("unchecked")
            PersistentTrie6Map<K, V> unchecked = (PersistentTrie6Map<K, V>) map;
            return unchecked;
        }
        PersistentTrie6Map.TransientTrieMap<K, V> tr = new PersistentTrie6Map.TransientTrieMap<>(of());
    ...
    public static <K, V> PersistentTrie6Map<K, V> of(@NonNull Map<? extends K, ? extends V> map) {
        return of(map.entrySet());
    }

    @SuppressWarnings("unchecked")
    private static <K, V> PersistentTrie6Map.Node<K, V> emptyNode() {
        return (PersistentTrie6Map.Node<K, V>) PersistentTrie6Map.EMPTY_NODE;
    }

    @SuppressWarnings("unchecked")
    public static <K, V> @NonNull PersistentTrie6Map<K, V> of() {
        return (PersistentTrie6Map<K, V>) PersistentTrie6Map.EMPTY_MAP;
    }

    public static <K, V> @NonNull PersistentTrie6Map<K, V> of(@NonNull Map.Entry<K, V>... entries) {
        PersistentTrie6Map.TransientTrieMap<K, V> result = PersistentTrie6Map.<K, V>of().asTransient();
    ...
    public static <K, V> @NonNull PersistentTrie6Map<K, V> of(@NonNull Iterable<? extends Map.Entry<? extends K, ? extends V>> entries) {
        TransientTrieMap<K, V> result = PersistentTrie6Map.<K, V>of().asTransient();
        for (Map.Entry<? extends K, ? extends V> entry : entries) {
            result.put(entry.getKey(), entry.getValue());
        }
        return result.freeze();
    ...
    public boolean equals(final Object other) {
        ...
        if (other instanceof PersistentTrie6Map) {
            PersistentTrie6Map<?, ?> that = (PersistentTrie6Map<?, ?>) other;
    ...
    public @NonNull PersistentTrie6Map<K, V> copyPut(@NonNull K key, @Nullable V value) {
        ...
        if (details.isModified()) {
            if (details.hasReplacedValue()) {
                ...
                return new PersistentTrie6Map<>(newRootNode,
                        hashCode + ((keyHash ^ valHashNew)) - ((keyHash ^ valHashOld)), size);
            }

            final int valHash = Objects.hashCode(value);
            return new PersistentTrie6Map<>(newRootNode, hashCode + ((keyHash ^ valHash)),
                    size + 1);
        }
    ...
    public @NonNull PersistentTrie6Map<K, V> copyPutAll(@NonNull Map<? extends K, ? extends V> map) {
    ...
    public @NonNull PersistentTrie6Map<K, V> copyRemove(@NonNull K key) {
        final int keyHash = Objects.hashCode(key);
        final ChangeEvent<V> details = new ChangeEvent<>();
    ...
    @Override
    public @NonNull PersistentTrie6Map<K, V> copyRemoveAll(@NonNull Iterable<? extends K> c) {
        final TransientTrieMap<K, V> t = this.asTransient();
        boolean modified = false;
        for (K key : c) {
    ...
    @Override
    public @NonNull PersistentTrie6Map<K, V> copyRetainAll(@NonNull Collection<? extends K> c) {
        final TransientTrieMap<K, V> t = this.asTransient();
        boolean modified = false;
            ...
            for (int idx = 0, n = payloadArity(); idx < n; idx++) {
                if (Objects.equals(getKey(idx), key)) {
                    ...
                    if (payloadArity() == 1) {
                        ...
                        return PersistentTrie6Map.<K, V>emptyNode().updated(bulkEdit, theOtherKey, theOtherVal,
                                keyHash, 0, details);
                    } else {
    ...
    static final class TransientTrieMap<K, V> {
        ...
        TransientTrieMap(PersistentTrie6Map<K, V> trieMap) {
        ...
        public PersistentTrie6Map<K, V> freeze() {
            bulkEdit = new PersistentTrieHelper.Nonce();
            return size == 0 ? PersistentTrie6Map.of() : new PersistentTrie6Map<>(root, hashCode, size);
        }

        public @NonNull PersistentTrie6Map.ChangeEvent<V> put(final K key, final V val) {
            final int keyHash = Objects.hashCode(key);
            final ChangeEvent<V> details = new ChangeEvent<>();
