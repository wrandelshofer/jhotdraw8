...
public class ChampTrie {
    ...
    static <K, V> HashCollisionNode<K, V> newHashCollisionNode(
            @Nullable UniqueIdentity mutator, int hash, @NonNull Object[] entries, int entryLength) {
        return mutator == null
     ...
     * @return the new root
     */
    public static <K, V> BitmapIndexedNode<K, V> renumber(int size, @NonNull BitmapIndexedNode<K, V> root, @NonNull UniqueIdentity mutator, int entryLength) {
        ...
        for (SequencedTrieIterator<K, V> i = new SequencedTrieIterator<>(size, root, entryLength); i.hasNext(); ) {
            ...
            newRoot = newRoot.update(mutator, entry.getKey(), entry.getValue(), Objects.hashCode(entry.getKey()), 0, details, entryLength, count++);
        }
        return newRoot;
    }
