...
public abstract class Node<K, V> {
    ...
    static <K, V> Node<K, V> mergeTwoDataEntrysIntoNode(UniqueIdentity mutator,
    ...
    abstract boolean equivalent(final @NonNull Object other, int entryLength, boolean hasSequenceNumber);
     ...
     */
    abstract Object findByKey(final K key, final int keyHash, final int shift, int entryLength);
    ...
    UniqueIdentity getMutator() {
        return null;
    }

    abstract Node<K, V> getNode(final int index, int entryLength);

    abstract V getValue(final int index, int entryLength);
    ...
    abstract Node<K, V> remove(final @Nullable UniqueIdentity mutator, final K key,
                               final int keyHash, final int shift, final ChangeEvent<V> details, int entryLength);

    abstract Node<K, V> update(final UniqueIdentity mutator, final K key, final V val,
