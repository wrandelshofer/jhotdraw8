...
public class UniqueShortestArcPathSearchAlgo<V, A> implements ArcPathSearchAlgo<V, A> {
    ...
    public @Nullable ArcBackLinkWithCost<V, A> search(
            ...
            int maxDepth,
            final int costLimit,
            final ToIntFunction3<V, V, A> costFunction, AddToSet<V> visited) {
        ...
        ArcBackLinkWithCost<V, A> found = null;
        while (!queue.isEmpty()) {
            ...
            if (u.getDepth() < maxDepth) {
                for (Arc<V, A> v : nextArcsFunction.apply(u.getVertex())) {
                    CostData costDataV = costMap.getOrDefault(v.getEnd(), infiniteCost);
                    final int bestKnownCost = costDataV.getCost();
                    int cost = (costToU + costf.apply(u.getVertex(), v.getEnd(), v.getArrow()));
        ...
        for (ArcBackLinkWithCost<V, A> node = found; node != null; node = node.getParent()) {
