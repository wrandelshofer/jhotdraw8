        ...
        for (; !pit.isDone(); pit.next()) {
            ...
            switch (type) {
                case PathIterator.SEG_CLOSE -> {
                    rayCheck = IntersectRayLine.intersectRayLineEx(aox, aoy, adx, ady, Double.MAX_VALUE, lastX, lastY, firstX, firstY, Scalars.REAL_THRESHOLD);
                }
                case PathIterator.SEG_CUBICTO -> {
                    x = coords[4];
                    y = coords[5];
                    rayCheck = IntersectRayCubicCurve.intersectRayCubicCurveEx(aox, aoy, adx, ady, Double.MAX_VALUE, lastX, lastY, coords[0], coords[1], coords[2], coords[3], x, y, Scalars.REAL_THRESHOLD);
                    lastX = x;
                    lastY = y;
                }
                case PathIterator.SEG_LINETO -> {
                    x = coords[0];
                    y = coords[1];
                    rayCheck = IntersectRayLine.intersectRayLineEx(aox, aoy, adx, ady, Double.MAX_VALUE, lastX, lastY, x, y, Scalars.REAL_THRESHOLD);
                    lastX = x;
                    lastY = y;
                }
                ...
                case PathIterator.SEG_QUADTO -> {
                    ...
                    rayCheck = IntersectRayQuadCurve.intersectRayQuadCurveEx(aox, aoy, adx, ady, Double.MAX_VALUE,
                            lastX, lastY, coords[0], coords[1], x, y, Scalars.REAL_THRESHOLD);
                    lastX = x;
                    lastY = y;
                }
                default -> {
